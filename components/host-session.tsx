    ...
    <div className="mx-auto min-h-screen w-full max-w-7xl px-5 py-6 sm:px-8">
      ...
      {snapshot.status === 'running' && config.mode !== 'values' && auction && item && <div className="grid gap-6 xl:grid-cols-[1fr_330px]">
        <div className="space-y-5">
          {auction.phase === 'planning' ? <Card className="shadow-sm"><CardHeader><CardTitle className="text-3xl">{t('planning')}</CardTitle></CardHeader><CardContent><p className="mb-5 text-muted-foreground">{t(config.mode === 'roles' ? 'rolePlanningHelp' : 'planningHelp')}</p>{config.mode === 'roles' && <RoleList roles={config.roles} />}<ol className="mt-5 space-y-2">{config.items.map((phrase, index) => <li key={phrase.id} className="flex gap-3 rounded-xl bg-muted/70 p-3"><span className="font-black text-primary">{index + 1}</span>{phrase.text}</li>)}</ol><Button size="lg" className="mt-6" onClick={() => startLot()}><Gavel />{t('startFirstLot')}</Button></CardContent></Card> : <>
