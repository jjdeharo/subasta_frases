...
import type { ActivityConfig, ClientSnapshot, Lang, LotResult, ParticipantPublic } from '@/lib/types';
...
export function HostSession({ config, lang, onExit }: Props) {
  ...
  function handleMessage(conn: DataConnection, raw: unknown) {
    ...
    if (message.type === 'hello' && message.token) {
      ...
      if (!participant) {
        ...
        participant = { id: message.token, token: message.token, name, online: true, balance: config.budget, score: 0, submitted: false, allocation: {} };
    ...
    if (message.type === 'bid' && config.mode === 'knowledge' && stateRef.current.knowledge.phase === 'bidding') {
  ...
  function closeBidding() {
    ...
    if (winner) { winner.balance -= auction.currentBid; winner.score += item.correct ? 1 : -1; }
    auction.lots.push({ itemId: item.id, winnerId: winner?.id ?? null, winnerName: winner?.name ?? null, amount: auction.currentBid, correct: item.correct });
  ...
  function exportCsv() {
    const rows = config.mode === 'knowledge'
      ? [['participant', 'points', 'balance'], ...snapshot.participants.map((p) => [p.name, p.score, p.balance])]
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-7xl px-5 py-6 sm:px-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <Button variant="ghost" onClick={onExit}><ArrowLeft />{t('back')}</Button>
      ...
      {snapshot.status === 'lobby' && <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
        <Card className="shadow-sm"><CardHeader><CardTitle>{t('session')}</CardTitle></CardHeader><CardContent className="text-center">
          ...
          <Button className="mt-4" variant="outline" disabled={!code} onClick={copyLink}>{copied ? <CheckCircle2 /> : <Copy />}{copied ? t('copied') : t('copy')}</Button>
        </CardContent></Card>
        <Card className="shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2"><Users />{t('waitingRoom')} <span className="rounded-full bg-secondary px-2 py-0.5 text-sm">{snapshot.participants.length}</span></CardTitle></CardHeader><CardContent>
          <ParticipantList participants={snapshot.participants} t={t} onRemove={removeParticipant} />
          <Button size="lg" className="mt-6 w-full" disabled={!code || snapshot.participants.length === 0} onClick={startActivity}><Play />{t('startActivity')}</Button>
        </CardContent></Card>
      </div>}

      {snapshot.status === 'running' && config.mode === 'knowledge' && auction && item && <div className="grid gap-6 xl:grid-cols-[1fr_330px]">
        <div className="space-y-5">
          {auction.phase === 'planning' ? <Card className="shadow-sm"><CardHeader><CardTitle className="text-3xl">{t('planning')}</CardTitle></CardHeader><CardContent><p className="mb-5 text-muted-foreground">{t('planningHelp')}</p><ol className="space-y-2">{config.items.map((phrase, index) => <li key={phrase.id} className="flex gap-3 rounded-xl bg-muted/70 p-3"><span className="font-black text-primary">{index + 1}</span>{phrase.text}</li>)}</ol><Button size="lg" className="mt-6" onClick={() => startLot()}><Gavel />{t('startFirstLot')}</Button></CardContent></Card> : <>
            ...
            {auction.phase === 'discussion' && <Button size="lg" className="w-full" onClick={openBidding}><Radio />{t('startBidding')}</Button>}
            {auction.phase === 'bidding' && <Button size="lg" className="w-full" onClick={closeBidding}><Square />{t('closeBidding')}</Button>}
            {auction.phase === 'revealed' && <Card className={item.correct ? 'bg-emerald-50 ring-emerald-700/20' : 'bg-rose-50 ring-rose-700/20'}><CardContent><h2 className={`text-2xl font-black ${item.correct ? 'text-emerald-800' : 'text-rose-800'}`}>{item.correct ? t('answerCorrect') : t('answerIncorrect')}</h2>{item.explanation && <p className="mt-3 text-lg leading-7">{item.explanation}</p>}<p className="mt-4 font-semibold">{auction.leaderName ? `${t('soldTo', { name: auction.leaderName, amount: auction.currentBid })} ${config.currencyName}` : t('unsold')}</p><Button className="mt-5" size="lg" onClick={nextLot}>{auction.index === config.items.length - 1 ? <Trophy /> : <Gavel />}{auction.index === config.items.length - 1 ? t('showFinalResults') : t('nextPhrase')}</Button></CardContent></Card>}
          </>}
        </div>
        <Card className="h-fit shadow-sm xl:sticky xl:top-5"><CardHeader><CardTitle>{t('rankings')}</CardTitle></CardHeader><CardContent><Ranking participants={sorted} currency={config.currencyName} t={t} /></CardContent></Card>
      </div>}
...
function makeSnapshot(config: ActivityConfig, state: InternalState): ClientSnapshot {
  ...
  const items = config.items.map((item, index) => ({ id: item.id, text: item.text, category: item.category, ...(revealCurrent && index === state.knowledge.index || state.status === 'finished' ? { correct: item.correct, explanation: item.explanation } : {}) }));
  const participants = [...state.participants.values()].map(({ id, name, online, balance, score, submitted }) => ({ id, name, online, balance, score, submitted }));
  ...
  return { revision: state.revision, status: state.status, config: { ...config, items }, participants, knowledge: config.mode === 'knowledge' ? { ...state.knowledge, leaderName: leader?.name ?? null } : undefined, values: config.mode === 'values' ? { open: state.values.open, totals, allocations } : undefined };
}

function ParticipantList({ participants, t, onRemove }: { participants: ParticipantPublic[]; t: (key: TranslationKey, replacements?: Record<string, string | number>) => string; onRemove: (id: string) => void }) {
  if (!participants.length) return <p className="rounded-xl bg-muted p-5 text-center text-muted-foreground">{t('noParticipants')}</p>;
  return <ul className="space-y-2">{participants.map((participant) => <li key={participant.id} className="flex items-center gap-3 rounded-xl border bg-white p-3"><span className={`size-2.5 rounded-full ${participant.online ? 'bg-emerald-500' : 'bg-stone-300'}`} /><span className="min-w-0 flex-1 truncate font-semibold">{participant.name}</span><span className="text-xs text-muted-foreground">{participant.online ? t('online') : t('offline')}</span><Button size="icon-sm" variant="ghost" aria-label={t('removeParticipant')} onClick={() => onRemove(participant.id)}><UserRoundX /></Button></li>)}</ul>;
}
function Ranking({ participants, currency, t }: { participants: ParticipantPublic[]; currency: string; t: (key: TranslationKey) => string }) { return <ol className="space-y-2">{participants.map((participant, index) => <li key={participant.id} className="grid grid-cols-[28px_1fr_auto] items-center gap-2 rounded-xl border bg-white p-3"><span className="font-black text-primary">{index + 1}</span><span className="truncate font-semibold">{participant.name}</span><span className="text-right text-sm"><strong>{participant.score} {t('points').toLowerCase()}</strong><br/><span className="text-muted-foreground">{participant.balance} {currency}</span></span></li>)}</ol>; }
...
function Results({ config, snapshot, t, onExport, onExit }: { config: ActivityConfig; snapshot: ClientSnapshot; t: (key: TranslationKey) => string; onExport: () => void; onExit: () => void }) {
  ...
  return <div className="mx-auto max-w-4xl"><div className="mb-8 text-center"><Trophy className="mx-auto size-14 text-amber-600"/><h1 className="mt-3 text-4xl font-black">{config.mode === 'knowledge' ? t('rankings') : t('collectiveResult')}</h1></div><Card className="shadow-sm"><CardContent>{config.mode === 'knowledge' ? <Ranking participants={sorted} currency={config.currencyName} t={t} /> : <div className="space-y-5">{valueRows.length ? valueRows.map((row, index) => <div key={row.id}><div className="mb-2 flex gap-3"><span className="font-black text-primary">{index + 1}</span><span className="flex-1 font-semibold">{row.text}</span><strong>{row.total} {config.currencyName}</strong></div><Progress value={row.total / max * 100} /></div>) : <p>{t('noAllocations')}</p>}</div>}</CardContent></Card>{snapshot.values?.allocations && <Card className="mt-6 shadow-sm"><CardHeader><CardTitle>{t('individualBreakdown')}</CardTitle></CardHeader><CardContent className="space-y-4">{Object.entries(snapshot.values.allocations).map(([name, allocation]) => <div key={name} className="rounded-xl border bg-white p-4"><h3 className="font-black">{name}</h3><ul className="mt-2 space-y-1 text-sm">{config.items.filter((item) => (allocation[item.id] || 0) > 0).map((item) => <li key={item.id} className="flex gap-3"><span className="flex-1">{item.text}</span><strong>{allocation[item.id]} {config.currencyName}</strong></li>)}</ul></div>)}</CardContent></Card>}<div className="mt-6 flex justify-center gap-3"><Button variant="outline" onClick={onExport}><Download />{t('exportCsv')}</Button><Button onClick={onExit}>{t('newActivity')}</Button></div></div>;
}
