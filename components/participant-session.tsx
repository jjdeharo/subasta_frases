...
export function ParticipantSession({ code, name, lang, onExit }: Props) {
  ...
  return <div className="mx-auto min-h-screen w-full max-w-3xl px-5 py-6 sm:px-8">
    ...
    {snapshot.status === 'finished' && <div><div className="text-center"><Trophy className="mx-auto size-14 text-amber-600"/><h1 className="mt-3 text-4xl font-black">{snapshot.config.mode !== 'values' ? t('rankings') : t('collectiveResult')}</h1></div>{snapshot.config.mode !== 'values' ? <><Card className="mt-7"><CardContent><ol className="space-y-2">{rankings.map((participant, index) => <li key={participant.id} className="grid grid-cols-[32px_1fr_auto] items-center rounded-xl border bg-white p-3"><strong className="text-primary">{index + 1}</strong><span className="font-semibold">{participant.name}{snapshot.config.mode === 'roles' && <span className="block"><RoleBadge role={snapshot.config.roles.find((role) => role.id === participant.roleId)}/></span>}</span><span className="text-right font-black">{participant.score}</span></li>)}</ol></CardContent></Card>{snapshot.config.mode === 'roles' && <RoleTotals snapshot={snapshot} t={t}/>}</> : <ValuesResult snapshot={snapshot} t={t} />}</div>}
  </div>;
}
