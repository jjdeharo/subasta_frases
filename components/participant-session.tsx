...
import { uid, type ClientSnapshot, type Lang } from '@/lib/types';
...
export function ParticipantSession({ code, name, lang, onExit }: Props) {
  ...
  if (status === 'error') return <Centered><WifiOff className="size-12 text-destructive"/><h1 className="mt-5 text-2xl font-black">{t('connectionError')}</h1><Button className="mt-5" onClick={onExit}>{t('back')}</Button></Centered>;
  if (!snapshot) return null;

  return <div className="mx-auto min-h-screen w-full max-w-3xl px-5 py-6 sm:px-8">
    <header className="mb-7 flex items-center justify-between gap-3"><Button variant="ghost" onClick={onExit}><ArrowLeft />{t('back')}</Button>{me && <div className="rounded-full bg-white px-4 py-2 text-sm font-bold shadow-sm">{me.name}</div>}</header>
    ...
    {snapshot.status === 'lobby' && <Centered><Users className="size-14 text-primary"/><h1 className="mt-5 text-3xl font-black">{t('waitingRoom')}</h1><p className="mt-2 text-center text-muted-foreground">{t('waitingForHost')}</p></Centered>}

    {snapshot.status === 'running' && snapshot.config.mode === 'knowledge' && auction && <>
      {auction.phase === 'planning' ? <div><h1 className="text-3xl font-black">{t('planning')}</h1><p className="mt-2 text-muted-foreground">{t('planningHelp')}</p><ol className="mt-6 space-y-3">{snapshot.config.items.map((phrase, index) => <li key={phrase.id} className="flex gap-3 rounded-xl border bg-white p-4 shadow-sm"><span className="font-black text-primary">{index + 1}</span><span>{phrase.text}</span></li>)}</ol></div> : item && <div className="space-y-5">
        ...
        {auction.phase === 'bidding' && <Card><CardContent><div className="flex items-end justify-between gap-4"><div><p className="text-sm text-muted-foreground">{t('currentBid')}</p><p className="text-4xl font-black">{auction.currentBid || '—'} <span className="text-sm text-muted-foreground">{snapshot.config.currencyName}</span></p><p className="mt-1 font-semibold">{auction.leaderName ? t('leader', { name: auction.leaderName }) : t('noBids')}</p></div><div className="text-right"><p className="text-sm text-muted-foreground">{t('balance')}</p><p className="text-2xl font-black">{me?.balance ?? snapshot.config.budget} <span className="text-xs text-muted-foreground">{snapshot.config.currencyName}</span></p></div></div><div className="mt-6 grid grid-cols-2 gap-3"><Button size="lg" disabled={!me || auction.leaderId === me.id} onClick={() => bid()}><Gavel />{t('bid', { amount: auction.currentBid === 0 ? snapshot.config.minBid : auction.currentBid + snapshot.config.increment })}</Button><Button size="lg" variant="outline" disabled={!me || auction.leaderId === me.id} onClick={() => bid(snapshot.config.increment * 2)}>+ {snapshot.config.increment * 2}</Button></div></CardContent></Card>}
    ...
    {snapshot.status === 'running' && snapshot.config.mode === 'values' && snapshot.values && <div>
      ...
      <Button size="lg" className="mt-5 w-full" disabled={!snapshot.values.open} onClick={submitAllocation}>{sent ? <CheckCircle2 /> : <Send />}{sent ? t('updateAllocation') : t('submitAllocation')}</Button>
    </div>}

    {snapshot.status === 'finished' && <div><div className="text-center"><Trophy className="mx-auto size-14 text-amber-600"/><h1 className="mt-3 text-4xl font-black">{snapshot.config.mode === 'knowledge' ? t('rankings') : t('collectiveResult')}</h1></div>{snapshot.config.mode === 'knowledge' ? <Card className="mt-7"><CardContent><ol className="space-y-2">{rankings.map((participant, index) => <li key={participant.id} className="grid grid-cols-[32px_1fr_auto] items-center rounded-xl border bg-white p-3"><strong className="text-primary">{index + 1}</strong><span className="font-semibold">{participant.name}</span><span className="text-right font-black">{participant.score}</span></li>)}</ol></CardContent></Card> : <ValuesResult snapshot={snapshot} t={t} />}</div>}
  </div>;
}
